		...
		int timeout_ms, void* p_decode, word16* packet_id)
{
	...
	case MQTT_PACKET_TYPE_PUBLISH:
	{
		...
		do {
			/* Determine if mqtt_message is done */
			msg_done = ((msg->buffer_pos + msg->buffer_len) >= msg->total_len) ? 1 : 0;

			if (msg->stat != MQTT_MSG_READ_PAYLOAD) {
				/* Issue callback for new mqtt_message */
				if (client->msg_cb) {
					if (!msg->buffer_new) {
						/* Reset topic name since valid on new mqtt_message only */
						msg->topic_name = NULL;
	...
	case MQTT_PACKET_TYPE_PUBLISH_COMP:
	{
		...
		/* Decode publish response mqtt_message */
	...
	default:
	{
		...
		PRINTF("MqttClient_Waitmqtt_message: Invalid client packet type %u!",
				msg->type);
#endif
		break;
	}
...
int MqttClient_Connect(MqttClient *client, MqttConnect *connect)
{
	...
	if(rc != 0){
		client->msg.stat = MQTT_MSG_BEGIN;
	}
...
int MqttClient_Publish(MqttClient *client, MqttPublish *publish)
{
	...
	case MQTT_MSG_WRITE:
	{
		...
		do {
			...
			/* Check if we are done sending publish mqtt_message */
...
int mqttclient_message_cb(MqttClient *client, MqttMessage *msg,
