		...
		int timeout_ms)
{
	...
	{
		rc = client->net->write( buf, buf_len,
				timeout_ms);
	}
...
#endif /* ENABLE_MQTT_TLS */
	{
		rc = client->net->read( buf, buf_len, timeout_ms);
...
//	unsigned char String_arr[50];
//	int sizez = 0;
//	EF_Number_to_String(buf , rc , String_arr , rc*2);
//
//	WIFI_Send_Arr("\nData : ", sizeof("\nData : "));
//	WIFI_Send_Arr(String_arr, rc*2);
//	WIFI_Send_Arr("\n", sizeof("\n"));
		...
		int timeout_ms, int use_tls, MqttTlsCb cb)
{
	...
	if ((client->flags & MQTT_CLIENT_FLAG_IS_CONNECTED) == 0) {
		...
		/* Connect to host */
		rc = client->net->connect( host, port, timeout_ms);
		if (rc < 0) {
			return rc;
		}
		client->flags |= MQTT_CLIENT_FLAG_IS_CONNECTED;
